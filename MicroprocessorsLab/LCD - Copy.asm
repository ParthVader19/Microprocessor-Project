#include p18f87k22.inc
    ...
    global  LCD_Setup, LCD_Write_Message, LCD_Write_Hex,LCD_Write_Hex_b,LCD_delay_x4us,movDRAM,LCD_Send_Byte_I,LCD_delay_ms
...
LCD_Setup
	...
	movlw	.10		; wait 40us
	call	LCD_delay_x4us
	movlw	b'00001111'	; display on, cursor on, blinking off
	call	LCD_Send_Byte_I
	movlw	.10		; wait 40us
	call	LCD_delay_x4us
	...
	movlw	.2		; wait 2ms
	call	LCD_delay_ms
	;movlw	b'00000100'	; entry mode incr by 1, shift
	;call	LCD_Send_Byte_I
	;movlw	.10		; wait 40us
	;call	LCD_delay_x4us
...
LCD_Hex_Nib	    ; writes low nibble as hex character
	...
	;this is to send binary to LCD
LCD_Write_Hex_b	    ; Writes byte stored in W as hex
	movwf	LCD_hex_tmp
	call	LCD_Send_Byte_D_b ; write out ascii
	return
...
LCD_Send_Byte_D_b		    ; Transmits byte stored in W to data reg
	movwf   LCD_tmp
	swapf   LCD_tmp,W   ; swap nibbles, high nibble goes first
	andlw   0x0f	    ; select just low nibble
	movwf   LATB	    ; output data bits to LCD
...
movDRAM	;subroutine intentended to move the cursor position, but can also be used to excute any other commond.
	call	LCD_Send_Byte_I
	movlw	.10		; wait 40us
	call	LCD_delay_x4us
	return
